    ...
    async fn seed_defaults(&self) -> Result<()> {
        ...
        for key in SettingsSnapshot::KEYS {
            let value = match *key {
                ...
                "precheck_gross_floor" => format!("{}", defaults.precheck_gross_floor),
    ...
    pub async fn get_settings(&self) -> Result<SettingsSnapshot> {
        ...
        for (key, value) in rows {
            match key.as_str() {
                ...
                "precheck_gross_floor" => snap.precheck_gross_floor = value.parse().unwrap_or(snap.precheck_gross_floor),
